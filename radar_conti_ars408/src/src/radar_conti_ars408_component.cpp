  ...
  const rclcpp_lifecycle::State &)
{
  ...
  auto radar_tracks_qos = rclcpp::QoS(rclcpp::KeepLast(5))
                            .reliability(rclcpp::ReliabilityPolicy::Reliable)
  ...
  do {
    ...
    if (node->get_parameter(radar_name + ".link_name", parameter)) {
      more_params = true;
      object_list_publishers_.push_back(this->create_publisher<radar_conti_ars408_msgs::msg::ObjectList>(
        parameter.as_string() + "/" + object_list_topic_name_, qos));
      tf_publishers_.push_back(
        this->create_publisher<tf2_msgs::msg::TFMessage>(parameter.as_string() + "/" + pub_tf_topic_name, qos));
      marker_array_publishers_.push_back(this->create_publisher<visualization_msgs::msg::MarkerArray>(
        parameter.as_string() + "/" + marker_array_topic_name_, qos));
      fov_marker_publishers_.push_back(
        this->create_publisher<visualization_msgs::msg::MarkerArray>(parameter.as_string() + "/fov", qos));
      fov_filter_marker_publishers_.push_back(
        this->create_publisher<visualization_msgs::msg::Marker>(parameter.as_string() + "/fov_filter", qos));
      radar_tracks_publishers_.push_back(this->create_publisher<radar_msgs::msg::RadarTracks>(
        parameter.as_string() + "/" + radar_tracks_topic_name_, radar_tracks_qos));
      obstacle_array_publishers_.push_back(this->create_publisher<nav2_dynamic_msgs::msg::ObstacleArray>(
        parameter.as_string() + "/" + obstacle_array_topic_name_, radar_tracks_qos));
      filter_config_publishers_.push_back(this->create_publisher<radar_conti_ars408_msgs::msg::FilterStateCfg>(
        parameter.as_string() + "/" + filter_config_topic_name_, transient_local_qos));
      radar_state_publishers_.push_back(this->create_publisher<radar_conti_ars408_msgs::msg::RadarState>(
        parameter.as_string() + "/" + radar_state_topic_name_, transient_local_qos));
      ...
      std::vector<bool> init_radar_active_values = {
      ...
      // NVM Storage
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_store_in_nvm"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_storeinnvm.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_store_in_nvm_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_storeinnvm_valid.data);

      // Ext Info
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_send_ext_info"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sendextinfo.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_send_ext_info_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sendextinfo_valid.data);

      // Ctrl Relay
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_ctrl_relay"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_ctrlrelay.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_ctrl_relay_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_ctrlrelay_valid.data);

      // Radar Power
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_radar_power"),
        START_RadarConfiguration_RadarCfg_RadarPower,
        radar_configuration_configs_[topic_ind].radarcfg_radarpower.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_radar_power_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_radarpower_valid.data);

      // Send Quality
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_send_quality"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sendquality.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_send_quality_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sendquality_valid.data);

      // Max Distance
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_max_distance"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_maxdistance.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_max_distance_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_maxdistance_valid.data);

      // Sensor ID
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_sensor_id"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sensorid.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_sensor_id_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sensorid_valid.data);

      // RCS Threshold
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_rcs_threshold"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_rcs_threshold.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_rcs_threshold_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_rcs_threshold_valid.data);

      // Output Type
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_output_type"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_outputtype.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_output_type_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_outputtype_valid.data);

      // Sort Index
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_sort_index"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sortindex.data);
      initializeConfig<uint8_t>(
        radar_name,
        std::string("radarcfg_sort_index_valid"),
        0,
        radar_configuration_configs_[topic_ind].radarcfg_sortindex_valid.data);

      // FILTER CONFIGS
      // Initialize Number of Objects Filters
      initializeConfig<uint32_t>(
        radar_name, std::string("filtercfg_min_nofobj"), 0, radar_filter_configs_[topic_ind].nofobj.min);
      initializeConfig<uint32_t>(
        radar_name, std::string("filtercfg_max_nofobj"), 20, radar_filter_configs_[topic_ind].nofobj.max);

      // Initialize Distance Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_distance"), 0.0, radar_filter_configs_[topic_ind].distance.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_distance"), 409.0, radar_filter_configs_[topic_ind].distance.max);

      // Initialize Azimuth Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_azimuth"), -50.0, radar_filter_configs_[topic_ind].azimuth.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_azimuth"), 50.0, radar_filter_configs_[topic_ind].azimuth.max);

      // Initialize Oncoming Velocity Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_vreloncome"), 0.0, radar_filter_configs_[topic_ind].vreloncome.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_vreloncome"), 128.0, radar_filter_configs_[topic_ind].vreloncome.max);

      // Initialize Departing Velocity Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_vreldepart"), 0.0, radar_filter_configs_[topic_ind].vreldepart.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_vreldepart"), 128.0, radar_filter_configs_[topic_ind].vreldepart.max);

      // Initialize Radar Cross Section Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_rcs"), -20.0, radar_filter_configs_[topic_ind].rcs.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_rcs"), 30.0, radar_filter_configs_[topic_ind].rcs.max);

      // Initialize Lifetime Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_lifetime"), 0.0, radar_filter_configs_[topic_ind].lifetime.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_lifetime"), 409.0, radar_filter_configs_[topic_ind].lifetime.max);

      // Initialize Size Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_size"), 0.0, radar_filter_configs_[topic_ind].size.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_size"), 102.0, radar_filter_configs_[topic_ind].size.max);

      // Initialize Probability of Existence Filters
      initializeConfig<uint32_t>(
        radar_name, std::string("filtercfg_min_probexists"), 0, radar_filter_configs_[topic_ind].probexists.min);
      initializeConfig<uint32_t>(
        radar_name, std::string("filtercfg_max_probexists"), 7, radar_filter_configs_[topic_ind].probexists.max);

      // Initialize Y Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_y"), -400.0, radar_filter_configs_[topic_ind].y.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_y"), 400.0, radar_filter_configs_[topic_ind].y.max);
      ...
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_vyrightleft"), 0.0, radar_filter_configs_[topic_ind].vyrightleft.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_vyrightleft"), 128.0, radar_filter_configs_[topic_ind].vyrightleft.max);

      // Initialize X Velocity oncoming Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_vxoncome"), 0.0, radar_filter_configs_[topic_ind].vxoncome.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_vxoncome"), 128.0, radar_filter_configs_[topic_ind].vxoncome.max);

      // Initialize Y Velocity going right to left Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_vyleftright"), 0.0, radar_filter_configs_[topic_ind].vyleftright.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_vyleftright"), 128.0, radar_filter_configs_[topic_ind].vyleftright.max);

      // Initialize X Velocity Departing Filters
      initializeConfig<float>(
        radar_name, std::string("filtercfg_min_vxdepart"), 0.0, radar_filter_configs_[topic_ind].vxdepart.min);
      initializeConfig<float>(
        radar_name, std::string("filtercfg_max_vxdepart"), 128.0, radar_filter_configs_[topic_ind].vxdepart.max);
  ...
  set_filter_service_ = create_service<radar_conti_ars408_msgs::srv::SetFilter>(
    "/radar_conti_ars408/set_filter",
    std::bind(&radar_conti_ars408::setFilterService, this, std::placeholders::_1, std::placeholders::_2));
  radar_config_service_ = create_service<radar_conti_ars408_msgs::srv::TriggerSetCfg>(
    "/radar_conti_ars408/set_radar_configuration",
    std::bind(&radar_conti_ars408::setRadarConfigurationService, this, std::placeholders::_1, std::placeholders::_2));
  if (!odom_topic_name_.empty()) {
    ...
    odometry_subscriber_ = create_subscription<nav_msgs::msg::Odometry>(
      odom_topic_name_, qos_settings, std::bind(&radar_conti_ars408::odomCallback, this, std::placeholders::_1));
  }
...
void radar_conti_ars408::publishRadarState(
  std::shared_ptr<const polymath::socketcan::CanFrame> frame, const int & sensor_id)
{
  ...
  radar_state_msg.nvmwritestatus =
    CALC_RadarState_RadarState_NVMwriteStatus(GET_RadarState_RadarState_NVMwriteStatus(frame->get_data()), 1.0);
  radar_state_msg.nvmreadstatus =
    CALC_RadarState_RadarState_NVMReadStatus(GET_RadarState_RadarState_NVMReadStatus(frame->get_data()), 1.0);
  radar_state_msg.maxdistancecfg =
    CALC_RadarState_RadarState_MaxDistanceCfg(GET_RadarState_RadarState_MaxDistanceCfg(frame->get_data()), 1.0);
  radar_state_msg.persistent_error =
    CALC_RadarState_RadarState_Persistent_Error(GET_RadarState_RadarState_Persistent_Error(frame->get_data()), 1.0);
  radar_state_msg.interference =
    CALC_RadarState_RadarState_Interference(GET_RadarState_RadarState_Interference(frame->get_data()), 1.0);
  radar_state_msg.temperature_error =
    CALC_RadarState_RadarState_Temperature_Error(GET_RadarState_RadarState_Temperature_Error(frame->get_data()), 1.0);
  radar_state_msg.temporary_error =
    CALC_RadarState_RadarState_Temporary_Error(GET_RadarState_RadarState_Temporary_Error(frame->get_data()), 1.0);
  radar_state_msg.voltage_error =
    CALC_RadarState_RadarState_Voltage_Error(GET_RadarState_RadarState_Voltage_Error(frame->get_data()), 1.0);
  radar_state_msg.radarpowercfg =
    CALC_RadarState_RadarState_RadarPowerCfg(GET_RadarState_RadarState_RadarPowerCfg(frame->get_data()), 1.0);
  radar_state_msg.sortindex =
    CALC_RadarState_RadarState_SortIndex(GET_RadarState_RadarState_SortIndex(frame->get_data()), 1.0);
  radar_state_msg.sensorid =
    CALC_RadarState_RadarState_SensorID(GET_RadarState_RadarState_SensorID(frame->get_data()), 1.0);
  radar_state_msg.motionrxstate =
    CALC_RadarState_RadarState_MotionRxState(GET_RadarState_RadarState_MotionRxState(frame->get_data()), 1.0);
  radar_state_msg.sendextinfocfg =
    CALC_RadarState_RadarState_SendExtInfoCfg(GET_RadarState_RadarState_SendExtInfoCfg(frame->get_data()), 1.0);
  radar_state_msg.sendqualitycfg =
    CALC_RadarState_RadarState_SendQualityCfg(GET_RadarState_RadarState_SendQualityCfg(frame->get_data()), 1.0);
  radar_state_msg.outputtypecfg =
    CALC_RadarState_RadarState_OutputTypeCfg(GET_RadarState_RadarState_OutputTypeCfg(frame->get_data()), 1.0);
  radar_state_msg.ctrlrelaycfg =
    CALC_RadarState_RadarState_CtrlRelayCfg(GET_RadarState_RadarState_CtrlRelayCfg(frame->get_data()), 1.0);
  radar_state_msg.rcs_threshold =
    CALC_RadarState_RadarState_RCS_Threshold(GET_RadarState_RadarState_RCS_Threshold(frame->get_data()), 1.0);
...
void radar_conti_ars408::handle_object_list(std::shared_ptr<const polymath::socketcan::CanFrame> frame)
{
  ...
  if (Get_MsgID0_From_MsgID(frame->get_id()) == ID_Obj_1_General) {
    ...
    o.object_general.obj_rcs.data = CALC_Obj_1_General_Obj_RCS(GET_Obj_1_General_Obj_RCS(frame->get_data()), 1.0);
...
void radar_conti_ars408::publish_object_map(int sensor_id)
{
  ...
  if (!odom_topic_name_.empty()) {
    corrected_odom = radar_transforms::transform2DOdom(
      vehicle_odometry_,
      tf_buffer_,
  ...
  for (itr = object_map_list_[sensor_id].begin(); itr != object_map_list_[sensor_id].end(); ++itr) {
    ...
    radar_track.position.x =
      (itr->second.object_extended.obj_length.data / 2) + itr->second.object_general.obj_distlong.data;
    ...
    radar_track.velocity =
      radar_transforms::correctObstacleVelocity(corrected_odom, radar_track_vel_raw, radar_track.position);
...
bool radar_conti_ars408::setFilter(
  const int & sensor_id,
  const int & active,
  const int & type,
  const int & index,
  ...
  switch (filterTypes[index]) {
    ...
    case FilterType::AZIMUTH:
      ...
      SET_FilterCfg_FilterCfg_Max_Azimuth(
        data, (max_value + FilterConfig::AZIMUTH_OFFSET) / FilterConfig::AZIMUTH_RESOLUTION);
      SET_FilterCfg_FilterCfg_Min_Azimuth(
        data, (min_value + FilterConfig::AZIMUTH_OFFSET) / FilterConfig::AZIMUTH_RESOLUTION);
      break;
    case FilterType::VRELONCOME:
...
bool radar_conti_ars408::setRadarConfiguration(
...
void radar_conti_ars408::updateFilterConfig(
  std::shared_ptr<const polymath::socketcan::CanFrame> frame, const int & sensor_id)
{
  ...
  switch (filterTypes[index]) {
    case FilterType::NOFOBJ:
      radar_filter_configs_[sensor_id].nofobj.min =
        CALC_FilterState_Cfg_FilterState_Min_NofObj(GET_FilterState_Cfg_FilterState_Min_NofObj(frame->get_data()), 1.0);
      radar_filter_configs_[sensor_id].nofobj.max =
        CALC_FilterState_Cfg_FilterState_Max_NofObj(GET_FilterState_Cfg_FilterState_Max_NofObj(frame->get_data()), 1.0);
    ...
    case FilterType::VRELDEPART:
      ...
      break;
    case FilterType::RCS:
      radar_filter_configs_[sensor_id].rcs.min =
        CALC_FilterState_Cfg_FilterState_Min_RCS(GET_FilterState_Cfg_FilterState_Min_RCS(frame->get_data()), 1.0);
      radar_filter_configs_[sensor_id].rcs.max =
        CALC_FilterState_Cfg_FilterState_Max_RCS(GET_FilterState_Cfg_FilterState_Max_RCS(frame->get_data()), 1.0);
    ...
    case FilterType::LIFETIME:
      ...
      break;
    case FilterType::SIZE:
      radar_filter_configs_[sensor_id].size.min =
        CALC_FilterState_Cfg_FilterState_Min_Size(GET_FilterState_Cfg_FilterState_Min_Size(frame->get_data()), 1.0);
      radar_filter_configs_[sensor_id].size.max =
        CALC_FilterState_Cfg_FilterState_Max_Size(GET_FilterState_Cfg_FilterState_Max_Size(frame->get_data()), 1.0);
    ...
    case FilterType::PROBEXISTS:
      ...
      break;
    case FilterType::Y:
      radar_filter_configs_[sensor_id].y.min =
        CALC_FilterState_Cfg_FilterState_Min_Y(GET_FilterState_Cfg_FilterState_Min_Y(frame->get_data()), 1.0);
      radar_filter_configs_[sensor_id].y.max =
        CALC_FilterState_Cfg_FilterState_Max_Y(GET_FilterState_Cfg_FilterState_Max_Y(frame->get_data()), 1.0);
      radar_filter_configs_[sensor_id].y.active = GET_FilterState_Cfg_FilterState_Active(frame->get_data());
      break;
    case FilterType::X:
      radar_filter_configs_[sensor_id].x.min =
        CALC_FilterState_Cfg_FilterState_Min_X(GET_FilterState_Cfg_FilterState_Min_X(frame->get_data()), 1.0);
      radar_filter_configs_[sensor_id].x.max =
        CALC_FilterState_Cfg_FilterState_Max_X(GET_FilterState_Cfg_FilterState_Max_X(frame->get_data()), 1.0);
...
void radar_conti_ars408::publishFovMetadata()
{
  for (size_t sensor_id = 0; sensor_id < radar_filter_configs_.size(); ++sensor_id) {
    ...
    auto fov_markers = radar_visualization::createRadarFOVMarkers(radar_link_names_[sensor_id], this->get_clock());
...
void radar_conti_ars408::sendMotionInputSignals(
  const size_t & sensor_id, radar_conti_ars408_structs::MotionInputSignal & motion_input_signal)
{
  ...
  SET_YawRateInformation_RadarDevice_YawRate(
    yaw_rate_info_data,
    (motion_input_signal.yaw_rate + YawRateInformation::YAW_RATE_OFFSET) / YawRateInformation::YAW_RATE_RESOLUTION);
