...
/**
   ...
   */
constexpr std::array<std::pair<int, int>, 10> pedestrian_fov = {
  {{8, 13}, {22, 20}, {32, 18}, {40, 15}, {50, 7}, {55, 9}, {80, 9}, {110, 0}, {110, 0}, {110, 0}}};

constexpr std::array<std::pair<int, int>, 10> moped_fov = {
  {{12, 22}, {23, 26}, {44, 25}, {58, 20}, {63, 12}, {70, 11}, {80, 12}, {120, 13}, {140, 10}, {170, 0}}};

constexpr std::array<std::pair<int, int>, 10> motorcycle_fov = {
  {{17, 27}, {42, 35}, {58, 31}, {65, 24}, {70, 11}, {110, 17}, {160, 12}, {190, 13}, {220, 0}, {220, 0}}};

constexpr std::array<std::pair<int, int>, 10> passenger_car_fov = {
  {{20, 35}, {53, 45}, {61, 35}, {70, 11}, {149, 24}, {210, 23}, {250, 17}, {250, 0}, {250, 0}, {250, 0}}};
...
visualization_msgs::msg::Marker createFovMarker(
  ...
  const std::array<std::pair<int, int>, 10> & points,
  const std::string & ns,
  int id,
  float r,
  float g,
...
visualization_msgs::msg::MarkerArray createRadarFOVMarkers(
  const std::string & radar_frame, rclcpp::Clock::SharedPtr clock)
{
  visualization_msgs::msg::MarkerArray marker_array;
  marker_array.markers.push_back(
    createFovMarker(radar_frame, pedestrian_fov, "pedestrian", 0, 1.0, 0.0, 1.0, 1.0, clock));  // Purple
  marker_array.markers.push_back(
    createFovMarker(radar_frame, moped_fov, "moped", 1, 0.0, 1.0, 0.0, 1.0, clock));  // Green
  marker_array.markers.push_back(
    createFovMarker(radar_frame, motorcycle_fov, "motorcycle", 2, 0.0, 0.0, 1.0, 1.0, clock));  // Blue
  marker_array.markers.push_back(
    createFovMarker(radar_frame, passenger_car_fov, "passenger_car", 3, 1.0, 0.0, 0.0, 1.0, clock));  // Red
...
/**
   ...
   */
visualization_msgs::msg::Marker createFilteredRadarFOVMarker(
  const std::string & radar_frame,
  double min_azimuth,
  double max_azimuth,
  double min_distance,
