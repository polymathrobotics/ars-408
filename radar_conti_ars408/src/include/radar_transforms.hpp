...
{
nav_msgs::msg::Odometry transform2DOdom(
  ...
  rclcpp::Clock::SharedPtr clock)
{
  ...
  try {
    base_link_to_sensor_transform =
      tf_buffer->lookupTransform(base_frame_id, sensor_frame_id, odometry.header.stamp, transform_timeout);
  } catch (tf2::TimeoutException & exception) {
    RCLCPP_ERROR_THROTTLE(
      rclcpp::get_logger("radar_conti_ars408"), *clock, 1000, "Transform timed out: %s", exception.what());
    return nav_msgs::msg::Odometry{};
  } catch (tf2::TransformException & exception) {
    RCLCPP_ERROR_THROTTLE(
      rclcpp::get_logger("radar_conti_ars408"), *clock, 1000, "Transform failed: %s", exception.what());
    return nav_msgs::msg::Odometry{};
  }
  ...
  transformed_odometry_base_link.twist.twist.linear.x =
    odometry.twist.twist.linear.x -
    odometry.twist.twist.angular.z * base_link_to_sensor_transform.transform.translation.y;
  transformed_odometry_base_link.twist.twist.linear.y =
    odometry.twist.twist.linear.y +
    odometry.twist.twist.angular.z * base_link_to_sensor_transform.transform.translation.x;
  ...
  transformed_odometry_sensor.twist.twist.linear.x = cos(-yaw) * transformed_odometry_base_link.twist.twist.linear.x -
                                                     sin(-yaw) * transformed_odometry_base_link.twist.twist.linear.y;
  transformed_odometry_sensor.twist.twist.linear.y = sin(-yaw) * transformed_odometry_base_link.twist.twist.linear.x +
                                                     cos(-yaw) * transformed_odometry_base_link.twist.twist.linear.y;
  transformed_odometry_base_link.twist.twist.angular.z = odometry.twist.twist.angular.z;
...
radar_conti_ars408_structs::MotionInputSignal createMotionInputSignal(
...
{
  auto corrected_odom =
    radar_transforms::transform2DOdom(odom, tf_buffer, sensor_frame_id, base_frame_id, transform_timeout, clock);
